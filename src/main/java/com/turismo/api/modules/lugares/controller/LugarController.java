...
// Módulo Lugares: exponer endpoints de lugares solo desde este controlador.
@RestController
...
public class LugarController {
    ...
    public ResponseEntity<ApiResponse<List<Lugar>>> listar() {
        return ResponseEntity.ok(
                ApiResponse.ok("Lugares encontrados", lugarService.listar())
        );
    }
    ...
    public ResponseEntity<ApiResponse<Lugar>> buscarPorId(@PathVariable Long id) {
        return ResponseEntity.ok(
                ApiResponse.ok("Lugar encontrado", lugarService.buscarPorId(id))
        );
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Lugar>> actualizar(
            @PathVariable Long id,
            @Valid @RequestBody Lugar lugar) {

        return ResponseEntity.ok(
                ApiResponse.ok("Lugar actualizado", lugarService.actualizar(id, lugar))
        );
    }
    ...
    public ResponseEntity<ApiResponse<Void>> eliminar(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(
                ApiResponse.ok("Lugar eliminado", null)
        );
    }
}